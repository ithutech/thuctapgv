                    ...
                    if (strPass == pass && strEmail == email && (strDT == sdt || strDT == sdt1)) {
                        ...
                        $.googleSheetToJSON('1nO2nV65Vi3dZWGlaIOXLEc-_JWEZK16XFbjQVH_3Q0U', 1)
                            .done(function (rows) {
                                ...
                                // var strGetPass = "<input type='password' name='pw_view_rp' hidden readonly>";
                                if (count == 0)
                                    ...
                                    // $("input[name=pw_view_rp]").val(strPass);
...
function xemBaoCao(masv) {
    ...
    model_error = "<h1 style='font-size: 150px; text-align: center'>OOPS !</h1> <h2 class='h2_error'>Lỗi! trang của bạn không tìm thấy hoặc mật khẩu đã thay đỗi, vui lòng đi tới trang chủ!</h2> <div class='go_to_home'><a href='/'>TRANG CHỦ</a></div>";
                ...
                if (strPass == pass && strEmail == email && (strDT == sdt || strDT == sdt1)) {
                      count++;
                        ...
                        //  $("input[name=txtGVSDT]").val(row[name]);
                          ...
                          strTextProfile += "<div class='form-wrapper'> <label for='txtGVEmail_Profile'>Mật khẩu</label> <input id='hash_input_pchange' type='password' placeholder='Mật khẩu mới?' class='form-control' onkeypress='return BlockSpace()' required=''  ><input type='password' id='hash_output_pchange' hidden readonly focus='disabled' style='cursor:not-allowed;'> </div>";
                          ...
                          strTextProfile += "<script type='text/javascript' src='./do_custom_info.js'></script>";
                  ...
                  if (count == 0) {
                      var model_error = "<h1 class='h1_error'>OOPS !</h1> <h2 class='h2_error'>Lỗi! trang của bạn không tìm thấy hoặc mật khẩu đã thay đỗi, vui lòng đi tới trang chủ!</h2> <div class='go_to_home'><a href='/'>TRANG CHỦ</a></div>";
                      $(".popup-content").html(model_error);
                  } else {
                      // strTextProfile += "</form>";
